...
const OurProcess = () => (
  <section className="py-20 bg-[#272727] bg-opacity-85 rounded-t-[2rem]">
    <div className="container text-center lg:mx-10">
      <h2 className="text-[3rem] font-bold mb-12 text-white neontext">Why Choose Us</h2>
      <div className="flex flex-wrap justify-between lg:mx-0 mx-10 gap-8">
        {steps.map((step, index) => (
          <div
            key={index}
            className="relative mt-10 lg:mt-0 flex-1 min-w-[250px] max-w-[300px] p-6 rounded-lg bg-black shadow-lg transform-gpu transition-transform duration-700 hover:rotate-x-6 hover:rotate-y-6"
          >
            <div className="relative flex flex-col items-center text-center h-full p-6">
              {/* Icon positioning */}
              <div className="absolute top-[-4rem] left-1/2 transform -translate-x-1/2 z-20 bg-black p-4 rounded-full shadow-md">
                {step.icon}
              </div>
              {/* Card content */}
